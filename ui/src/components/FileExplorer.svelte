		...
		<div class="overflow-auto w-full max-h-[200px] border-2">			
				<div class="pl-2 pb-2 h-fit text-[13px] font-semibold flex">
						...
						{#each currentPath as folder}
							<button onclick={()=>goToLevel(folder)} class="text-nowrap hover:underline"> {folder}/</button>
						{/each}
				</div>
                {#if navigationStack.length > 0 || onDetails}
				<button class="flex ml-2 italic underline hover:underline" onclick={goBack}
				> Return </button
				>
                {/if}
            <table class="ml-2">
                    ...
                    {#if onDetails == false}
                        {#each Object.keys(currentLevel) as item}
                            ...
                            {:else}
                                <tr>					
                                    <td class="pr-1">
                                        <button onclick={()=>goToDetails(currentLevel[item], item)}><p class="flex">{item}</p></button>
                                    </td>
                            <!-- <td class="px-1">
                                        <p>
